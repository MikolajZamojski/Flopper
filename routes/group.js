...
router.post('/:groupId/pfp', authenticateToken, authorizeGroupOwnership, pfpUpload.single('avatar'), async (req, res) => {
  const updateResult = (await req.dbConnect.collection("Groups").findOneAndUpdate({_id: req.params.groupId}, {$set : {"pfp-filename": (req.hashedFileName ?? null)}}, {upsert: true, projection: {"pfp-filename": 1, _id: 0}})).value["pfp-filename"];
  ...
  if(req.dir === undefined || req.hashedFileName === undefined) {
    return res.status(201).json({msg: "Profile picture reset."});
  }
