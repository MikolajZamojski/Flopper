...
router.post('/', authenticateToken, (req, res, next) => {req.postId = crypto.randomUUID().replace(/-/g, ''); next()}, postUploadFields , async(req, res) => {
  ...
  Object.keys(req.body).forEach((key) => {
    if(key !== "text") {
      if(req.body[key].substr(0, 25) === "https://open.spotify.com/")
...
router.put('/:postId/like', authenticateToken, async(req, res) => {
  ...
  if(likeResult !== null) {
    ...
    return res.status(201).json({isLiked: false, likesCount: post["likes-count"] - 1});
  }
  else {
    ...
    return res.status(201).json({isLiked: true, likesCount: post["likes-count"] + 1 || 1});
  }
})
